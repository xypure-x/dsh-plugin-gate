...
export function apply(ctx, config) {
    ...
    function trustedDirectory(path) {
        ...
        if (!canonical)
            return { ok: false, error: `目录不存在或无法解析真实路径: ${resolve(path)}` };
        const roots = trustedRoots.map(canonicalPath).filter((root) => root !== null);
        if (roots.length === 0)
            return { ok: false, error: '未配置 trustedRoots；为防止 AI 注入任意本地代码，当前拒绝外部插件目录' };
        if (!roots.some((root) => isWithinRoot(canonical, root))) {
            return { ok: false, error: `目录不在 trustedRoots 内: ${canonical}` };
        }
        return { ok: true, canonical };
    }
    function packageProblems(dir) {
        const problems = [];
        const pkgPath = join(dir, 'package.json');
        try {
            const pkg = JSON.parse(readFileSync(pkgPath, 'utf8'));
            if (typeof pkg.name !== 'string' || !pkg.name)
                problems.push('package.json 缺少有效 name');
            if (pkg.main !== undefined) {
                const main = canonicalPath(join(dir, String(pkg.main)));
                if (!main || !isWithinRoot(main, dir))
                    problems.push('package.json main 指向包目录外或不存在');
            }
        }
        catch {
    ...
    function matchesSelf(match) {
        if (String(match).includes('dsh-plugin-gate'))
            return true;
        for (const entry of ctx.loader.entries()) {
            const o = entry.options;
            if (o.group)
                continue;
            if (!String(o.name).includes('dsh-plugin-gate'))
                continue;
    ...
    function notifyClientRebuilt(name) {
        try {
            ...
            if (cm && typeof cm.rebuilt === 'function') {
                ...
                if (rev)
                    logger.info('[plugin-gate] client bundle 已联动（rev=%s）', rev);
            }
        }
    ...
    async function inject(dir) {
        ...
        if (!fingerprint)
            return 'ERROR: 无法为插件生成 lib 指纹，已拒绝持久化注入状态';
    ...
    async function uninject(match) {
        if (match.includes('plugin-gate'))
            return 'ERROR: 拒绝卸载 dsh-plugin-gate 自身（引导器不可卸载）';
        const steps = [];
        let fullName = null;
    ...
    ctx.setInterval(() => {
        ...
        for (const w of watchList) {
            ...
            if (String(w.match).includes('dsh-plugin-gate'))
                continue;
    ...
    safeRegister(defineTool({
        ...
        async execute(args) {
            return withOpLock(async () => {
                ...
                if (profileName !== 'web')
                    return 'ERROR: 安全版只允许修改 web profile';
                try {
                    const pkgPath = join(dir, 'package.json');
                    if (!existsSync(pkgPath))
    ...
    safeRegister(defineTool({
        ...
        async execute(args) {
            if (!config.allowRelease)
                return 'ERROR: 发布能力默认关闭，请在受控环境显式启用 allowRelease 后重试';
            const pluginDir = resolve(String(args.dir ?? '').trim());
            if (!pluginDir || !existsSync(join(pluginDir, 'package.json')))
                return 'ERROR: dir 无 package.json';
            const trusted = trustedDirectory(pluginDir);
        ...
        async execute() {
            ...
            try {
                ...
                if (inj.includes('host ✓')) {
                    ...
                    let diag = '';
                    try {
                        diag += 'junction→' + readlinkSync(join(profileNodeModules, '@xypure-x', TEST_SHORT)) + '\n';
                    }
                    catch (e) {
                ...
                try {
                    ...
                    if (!libPath) {
                        ...
                        for (const u of lcT?.keys() ?? []) {
                            ...
                            if (!d.includes('dsh-plugin-gate') && !d.includes('cloud-restore'))
                                continue;
    ...
    function apiAuthorized(req) {
        ...
        if (typeof header !== 'string' || header !== config.apiToken)
            return false;
            ...
            try {
                ...
                if (req.method === 'POST' && path === '/uninstall') {
                    if (!apiAuthorized(req))
                        return send(401, { ok: false, error: 'authentication required' });
