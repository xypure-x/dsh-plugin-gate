...
# Generated by dsh-plugin-gate dev_scaffold_plugin.
...
export function apply(ctx: AppContext, config: Config): void {
  ...
  const logFile = config.logFile || join(homedir(), '.dsh', 'plugin-gate', SHORT + '.log')
  const watchFile = config.watchFile || join(homedir(), '.dsh', 'plugin-gate', 'self-heal.log')
  let cycles = 0
  let llmCalls = 0
...
export const name = 'dsh-plugin-gate'
...
export const Config = z.object({
  ...
  allowUnsafeBuildScripts: z.boolean().default(false),
  allowRelease: z.boolean().default(false),
...
export function apply(ctx: AppContext, config: Config): void {
  ...
  const registryFile = config.registryFile || join(dshHome, 'plugin-gate', 'registry.json')
  ...
  const trustedRoots = (config.trustedRoots ?? []).map((root) => resolve(root)).filter((root, i, all) => all.indexOf(root) === i)

  function canonicalPath(path: string): string | null {
    try {
      return realpathSync(resolve(path)).replace(/\\/g, '/')
    } catch {
      return null
    }
  }
  ...
  function trustedDirectory(path: string): { ok: true; canonical: string } | { ok: false; error: string } {
    const canonical = canonicalPath(path)
    if (!canonical) return { ok: false, error: `目录不存在或无法解析真实路径: ${resolve(path)}` }
    const roots = trustedRoots.map(canonicalPath).filter((root): root is string => root !== null)
    if (roots.length === 0) return { ok: false, error: '未配置 trustedRoots；为防止 AI 注入任意本地代码，当前拒绝外部插件目录' }
    if (!roots.some((root) => isWithinRoot(canonical, root))) {
      return { ok: false, error: `目录不在 trustedRoots 内: ${canonical}` }
    }
    return { ok: true, canonical }
  }

  function packageProblems(dir: string): string[] {
    const problems: string[] = []
    const pkgPath = join(dir, 'package.json')
    try {
      const pkg = JSON.parse(readFileSync(pkgPath, 'utf8')) as { name?: unknown; main?: unknown }
      if (typeof pkg.name !== 'string' || !pkg.name) problems.push('package.json 缺少有效 name')
      if (pkg.main !== undefined) {
        const main = canonicalPath(join(dir, String(pkg.main)))
        if (!main || !isWithinRoot(main, dir)) problems.push('package.json main 指向包目录外或不存在')
      }
    } catch {
      problems.push('package.json 不存在或无法解析')
    }
    if (!existsSync(join(dir, 'lib', 'index.js'))) problems.push('缺少 lib/index.js')
    return problems
  }
  ...
  function writeRegistry(list: RegistryEntry[]): void {
    ...
    try { chmodSync(registryFile, 0o600) } catch { /* Windows or unsupported filesystem */ }
  }
  ...
  function matchesSelf(match: string): boolean {
    if (String(match).includes('dsh-plugin-gate')) return true
    for (const entry of ctx.loader.entries()) {
      const o = entry.options
      if (o.group) continue
      if (!String(o.name).includes('dsh-plugin-gate')) continue
  ...
  function scheduleHeal(selfEntry: any, reason: string): void {
    try {
      ...
      const pkgName = selfEntry?.options?.name ?? '@xypure-x/dsh-plugin-gate'
      ...
      const heal = (): void => {
        ...
        void (async () => {
          try {
              ...
              try {
                ...
                try {
                  for (const entry of rebootCtx.loader.entries()) {
                    const o = entry.options
                    if (o.group) continue
                    if (String(o.name).includes('dsh-plugin-gate') && entry.fiber && FIBER_NAMES[entry.fiber.state] === 'active') {
                      officialAlive = true
                    }
                  }
                } catch { /* 枚举失败按未复活处理 */ }
                if (officialAlive) {
                  logger.info('[plugin-gate] 自愈：第 %d 次 touch patch 官方重装配生效（%s）', attempt, pkgName)
                ...
                logger.info('[plugin-gate] 自愈：第 %d 次 fallback loader.create 重新装配完成（%s）', attempt, pkgName)
                opStats.selfHeal.ok += 1
                saveStats()
            ...
            logger.error('[plugin-gate] 自愈第 %d 次失败: %s', attempt, String(e))
      ...
      logger.warn('[plugin-gate] 自愈已排程（4s 后第 1 次：touch patch 官方装配，最多 3 次，触发=%s）', reason)
    } catch (e) {
      logger.error('[plugin-gate] 自愈排程失败: %s', String(e))
    }
  }
  ...
  async function reloadPackage(match: string, urlMatch?: string): Promise<string> {
    ...
    try {
      ...
      if (fresh.warn.length > 0) {
        ...
        logger.warn('[plugin-gate] 重载 %s 构建产物可能过期（未阻断）: %s', urlKey, fresh.warn.join('; '))
      }
      ...
      const planned = globalThis.setTimeout(() => {
        void (async () => {
          try {
            ...
            logger.info('[plugin-gate] 重启器重建完成（REPLACE 语义，fiber=%s）', entry.fiber?.state)
          } catch (error) {
            logger.error('[plugin-gate] 重启器重建失败: %s', String(error))
            console.error('[plugin-gate] 重启器重建失败:', error)
    ...
    for (const entry of ctx.loader.entries()) {
      ...
      if (String(o.name).includes(match) || String(o.id).includes(match)) {
        return 'ERROR: 重载目标命中插件自身，但未走自重载路径（匹配串需含 dsh-plugin-gate）。已拒绝——防止无保护自毁'
      }
    }
    ...
    try {
      ...
      appendFileSync(join(dshHome, 'plugin-gate', 'reload-debug.log'), dbg.join('\n') + '\n')
    } catch { /* 诊断失败不阻塞 */ }
  ...
  function saveStaging(): void {
    // Hardened build intentionally does not persist executable staging source.
  }

  /** 恢复持久化的 staging（含 promoted 重新转正注册）。 */
  function restoreStaging(): void {
    // Hardened build intentionally never restores executable staging source.
  }
  restoreStaging()

  safeRegister(defineTool({
    name: 'dev_stage_add',
    description: '安全版默认禁用动态脚本 staging；请开发静态插件并通过 trustedRoots 注入。',
    parameters: {
      ...
      execute: { type: 'string', required: true, description: '已禁用：不接受可执行源码' },
    },
    output: { schema: { type: 'string' }, render: (_a: unknown, v: unknown) => [{ type: 'text', text: String(v) }] },
    async execute(args: any) {
      void args
      return 'ERROR: 安全版已禁用任意 JavaScript staging；请创建静态插件并置于 trustedRoots 下'
    },
  }))
  ...
  function cleanupStaleEntries(name: string): void {
    for (const entry of ctx.loader.entries()) {
      ...
      try {
        ...
        logger.info('[plugin-gate] 清理残留 entry %s（%s）', entry.id, st)
      } catch { /* 清理失败不阻塞 */ }
    }
  }
  ...
  function refreshClientRow(name: string): void {
    try {
      ...
      if (changed && typeof cm.compose === 'function' && typeof cm.notifyGraphChanged === 'function') {
        ...
        logger.info('[plugin-gate] client 模块已注册 %s', name)
      }
    } catch (e) {
      logger.warn('[plugin-gate] client 模块补扫失败: %s', String(e))
    }
  }
  ...
  function notifyClientRebuilt(name: string): void {
    try {
      ...
      if (cm && typeof cm.rebuilt === 'function') {
        ...
        if (rev) logger.info('[plugin-gate] client bundle 已联动（rev=%s）', rev)
      }
  ...
  function removeClientRow(name: string): void {
    try {
      ...
      if (cm.table.delete(name)) {
        ...
        logger.info('[plugin-gate] client 模块表已移除 %s', name)
      }
    } catch { /* 清理失败不阻塞 */ }
  }
  ...
  function saveStats(): void {
    try {
      mkdirSync(dirname(statsFile), { recursive: true, mode: 0o700 })
      writeFileSync(statsFile, JSON.stringify(opStats, null, 2), 'utf8')
      try { chmodSync(statsFile, 0o600) } catch { /* Windows or unsupported filesystem */ }
    } catch { /* 统计写失败不阻塞 */ }
  }
  ...
  function arbitrateOfficial(): void {
    try {
      const entries = [...ctx.loader.entries()].filter((e) => !e.options.group && String(e.options.name).includes('dsh-plugin-gate'))
  ...
  async function inject(dir: string): Promise<string> {
    const trusted = trustedDirectory(dir)
    if (!trusted.ok) return 'ERROR: ' + trusted.error
    const absDir = trusted.canonical
    ...
    const packageIssues = packageProblems(absDir)
    if (packageIssues.length > 0) return 'ERROR: 插件包校验失败（已拒绝注入）：\n- ' + packageIssues.join('\n- ')
    let pkg: { name?: string }
    try {
    ...
    const fingerprint = fingerprintOf(join(absDir, 'lib'))
    if (!fingerprint) return 'ERROR: 无法为插件生成 lib 指纹，已拒绝持久化注入状态'
    const existing = list.find((e) => e.dir === absDir)
    if (existing) {
      existing.name = pkgName
      existing.fingerprint = fingerprint
      writeRegistry(list)
    } else {
      list.push({ dir: absDir, name: pkgName, at: new Date().toISOString(), fingerprint })
      writeRegistry(list)
    }
  ...
  async function uninject(match: string): Promise<string> {
    if (match.includes('plugin-gate')) return 'ERROR: 拒绝卸载 dsh-plugin-gate 自身（引导器不可卸载）'
    const steps: string[] = []
      ...
      for (const name of linkNames) {
        ...
        if (!isHealthyLink(linkPath)) {
          ...
          try {
            ...
            logger.warn('[plugin-gate] 断电自愈：重建 junction %s → %s', name, target)
          } catch (err) {
            logger.warn('[plugin-gate] junction 重建失败 %s: %s', name, String(err))
          }
        }
      }
      ...
      for (const name of bundles) {
        if (bundleSet.has(name) && !linkNames.includes(name)) {
          ...
          if (!dep.startsWith('link:') && !isHealthyLink(join(profileNodeModules, ...(name.startsWith('@') ? name.split('/') : [name])))) {
            logger.warn('[plugin-gate] bundle %s 非 link 依赖且 junction 异常（registry 包由包管理器管理）', name)
          }
        }
      }
    } catch (err) {
      logger.warn('[plugin-gate] link 依赖 junction 自愈扫描失败: %s', String(err))
    }
    return healed
  }
  ...
  async function restore(): Promise<void> {    // ① profile link 依赖 junction 自愈（断电/强制关机后 junction 悬空 → 重建；
    ...
    try {
      ...
      logger.warn('[plugin-gate] junction 自愈扫描失败: %s', String(err))
    }
    // ② 注入清单恢复（原逻辑）
    for (const e of readRegistry()) {
      try {
        if (hasActiveEntry(e.name)) continue
        if (!e.fingerprint || fingerprintOf(join(e.dir, 'lib')) !== e.fingerprint) {
          auditLog('restore-skip-fingerprint', `${e.name} 指纹缺失或已变化，拒绝自动恢复`)
          logger.warn('[plugin-gate] 恢复 %s 跳过（插件指纹缺失或已变化）', e.name)
          continue
        }
        ...
        if (block.length > 0) {
          ...
          logger.warn('[plugin-gate] 恢复 %s 跳过（client 骨架/构建产物问题）: %s', e.name, block.join('; '))
          continue
        }
        await inject(e.dir)
        logger.info('[plugin-gate] 自动恢复 %s', e.name)
      } catch (err) {
        logger.warn('[plugin-gate] 恢复 %s 失败: %s', e.name, err instanceof Error ? err.stack : String(err))
      }
    }
  }
  ...
  ctx.setInterval(() => {
    ...
    for (const w of watchList) {
      ...
      if (String(w.match).includes('dsh-plugin-gate')) continue
      ...
      if (prev !== undefined && prev !== fp) {
        ...
        precheck.then((probeOk: boolean) => {
          if (!probeOk) {
            ...
            logger.warn('[plugin-gate] 自动重载预检失败，跳过（旧代码继续运行）: %s', w.dir)
            return
          }
          // match 用于 entry 查找，dir 用于 URL 匹配（loadCache key 是 file URL）
          reloadPackage(w.match, w.dir)
            .then(r => logger.info('[plugin-gate] %s', r))
            .catch(e => logger.warn('[plugin-gate] %s', e instanceof Error ? e.stack : String(e)))
  ...
  function safeRegister(tool: any): void {
    try {
      ctx.effect(() => ctx.tools.register(tool), `dsh-plugin-gate: ${tool.name ?? 'tool'}`)
    } catch (e) {
      logger.warn('[plugin-gate] 跳过冲突工具注册: %s', e instanceof Error ? e.message : String(e))
    }
  }
  ...
  function purgeStaleTools(): void {
    try {
      ...
      if (names.length > 0) {
        logger.warn('[plugin-gate] 已清理 %d 个僵尸工具残留: %s', names.length, names.join(','))
  ...
  safeRegister(defineTool({
    ...
    async execute(args: { dir: string }) {
      ...
      if (fresh.warn.length > 0) {
        ...
        logger.warn('[plugin-gate] 注入 %s 构建产物可能过期（未阻断）: %s', dir, fresh.warn.join('; '))
      }
  ...
  safeRegister(defineTool({
    ...
    parameters: {
      match: { type: 'string', required: true, description: '包名/路径子串（如 dsh-toy-supermod 或 @xypure-x/dsh-toy-supermod）' },
    },
    output: {
      schema: { type: 'string' },
      render: (_args: unknown, value: unknown) => [{ type: 'text', text: String(value) }],
  ...
  safeRegister(defineTool({
    ...
    async execute(args) {
      ...
      const requestedDir = String(args.dir)
      const trusted = trustedDirectory(requestedDir)
      if (!trusted.ok) return 'ERROR: ' + trusted.error
      const dir = trusted.canonical
      const profileName = args.profile ? String(args.profile) : 'web'
      if (profileName !== 'web') return 'ERROR: 安全版只允许修改 web profile'
      try {
        const pkgPath = join(dir, 'package.json')
        if (!existsSync(pkgPath)) return 'ERROR: 目录无 package.json: ' + dir
        const packageIssues = packageProblems(dir)
        if (packageIssues.length > 0) return 'ERROR: 插件包校验失败：\n- ' + packageIssues.join('\n- ')
  ...
  safeRegister(defineTool({
    ...
    async execute(args: { dir: string; name: string; form?: string; description?: string }) {
      ...
      const pkgName = rawName.startsWith('@') ? rawName : '@xypure-x/' + rawName
      try {
  ...
  safeRegister(defineTool({
    ...
    async execute(args: { dir: string }) {
      ...
      const trusted = trustedDirectory(pluginDir)
      if (!trusted.ok) return 'ERROR: ' + trusted.error
      if (!config.allowUnsafeBuildScripts) {
        return 'ERROR: 默认拒绝执行插件目录中的 build.sh/npm 脚本。请先在受控环境显式启用 allowUnsafeBuildScripts，并确认该目录可信。'
      }
  ...
  safeRegister(defineTool({
    ...
    async execute(args: { dir: string; version?: string; notes?: string }) {
      if (!config.allowRelease) return 'ERROR: 发布能力默认关闭，请在受控环境显式启用 allowRelease 后重试'
      const pluginDir = resolve(String(args.dir ?? '').trim())
      if (!pluginDir || !existsSync(join(pluginDir, 'package.json'))) return 'ERROR: dir 无 package.json'
      const trusted = trustedDirectory(pluginDir)
      if (!trusted.ok) return 'ERROR: ' + trusted.error
  ...
  safeRegister(defineTool({
    ...
    async execute() {
      ...
      // 实测踩坑）。全新 specifier + 固定目录 → 首次解析后缓存永远一致。
       const TEST_PKG = '@xypure-x/selftest-runner'
      const TEST_SHORT = 'selftest-runner'
        const tmpDir = join(tmpdir(), 'dsh-plugin-gate', TEST_SHORT + '-' + randomUUID())
      ...
      try {
        ...
        if (inj.includes('host ✓')) {
          ...
          let diag = ''
           try { diag += 'junction→' + readlinkSync(join(profileNodeModules, '@xypure-x', TEST_SHORT)) + '\n' } catch (e) { diag += 'junction err: ' + (e as { code?: string }).code + '\n' }
        ...
        writeSelfReloadState(Date.now() - 5000)
         const throttle = await withOpLock(() => reloadPackage('dsh-plugin-gate'))
        ...
        let libPath = ''
        try {
           const juncLib = join(profileNodeModules, '@xypure-x', 'dsh-plugin-gate', 'lib', 'index.js')
          ...
          if (!libPath) {
            ...
            for (const u of lcT?.keys() ?? []) {
              ...
              const d = decodeURIComponent(u)
               if (!d.includes('dsh-plugin-gate') && !d.includes('cloud-restore')) continue
        ...
        if (libPath) {
          ...
          try {
            ...
            writeFileSync(libPath, 'BROKEN SELF TEST !!!', 'utf8')
             const precheck = await withOpLock(() => reloadPackage('dsh-plugin-gate'))
        ...
        try {
           const linkDir = join(profileNodeModules, '@xypure-x', TEST_SHORT)
  ...
  try {
    ctx.systemPrompt.context({
      ...
      // 1) dev_* 工具 = dsh-plugin-gate（受控运行时插件通道）；
    ...
    text: '本环境装有 dsh-plugin-gate（dev_* 工具）：受 trustedRoots 和安全策略保护的本地插件可运行时管理——工具包/守护循环/UI 面板/混合形态；支持受控注入、热重载、卸载和状态审计。默认禁止动态 JavaScript staging、自动 ingest、任意构建脚本和发布操作。从零体验路径：dev_plugin_status → dev_self_test → dev_scaffold_plugin → dev_build_plugin → dev_inject_plugin → dev_uninject_plugin。',
    })
  } catch (e) {
    logger.warn('[plugin-gate] systemPrompt.context 重复注册容忍（跳过，新实例继续运行）: %s', e instanceof Error ? e.message : String(e))
  }

  logger.info('[plugin-gate] 就绪：watch %d 目录（%dms），autoRestore=%s', watches.length, intervalMs, String(config.autoRestore))
  ...
  try {
    ...
    if (cmSvc?.pkgMeta && typeof cmSvc.pkgMeta.delete === 'function') {
       cmSvc.pkgMeta.delete('@xypure-x/dsh-plugin-gate')
      if (typeof cmSvc.processOne === 'function') {
         cmSvc.processOne('@xypure-x/dsh-plugin-gate')
  ...
  function apiAuthorized(req: any): boolean {
    if (!config.apiToken) return false
       const header = req?.headers?.['x-dsh-plugin-gate-token']
    if (typeof header !== 'string' || header !== config.apiToken) return false
  ...
  async function startIngest(dir: string, title?: string): Promise<string> {
    void dir
    void title
    return 'ERROR: 安全版默认禁用自动 ingest；请先完成只读审查，并通过用户确认后手动构建和注入'
  }

  ctx.effect(() => ctx.webServer.register({
    kind: 'prefix',
     path: '/plugin-gate/api',
    handler: async (req: any, res: any) => {
      ...
      try {
        const url = new URL(req.url ?? '/', 'http://localhost')
         const path = url.pathname.replace(/^\/plugin-gate\/api/, '') || '/'
        ...
        if (req.method === 'POST' && path === '/uninstall') {
          if (!apiAuthorized(req)) return send(401, { ok: false, error: 'authentication required' })
   ...
   }), 'plugin-gate: plugin-manager-api')
}
